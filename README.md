...
- `/quiz` — тренировка: вопрос → «показать ответ» → честная самооценка
  (❌ не знал / 🤔 частично / ✅ знал). Оценка идёт в spaced repetition и
  определяет, когда вопрос вернётся.
- `/stats` — прогресс и покрытие по темам roadmap.
- `/subscribe` — «вопрос дня»: бот сам присылает вопрос утром по расписанию.
- Банк вопросов и дерево тем — в `data/questions.yaml` (легко дополнять).
